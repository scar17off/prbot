...
function createBot() {
	...
	bot.on("message", msg => {
		...
		console.log(msget);
		let user = args[2].toString();
		if(args[3] == `quit`){
        if(user.includes(bossName)){
		  	bot.quit();
        return false;
		  };
    };
